...
Future<void> _downloadPdf(BuildContext context) async {
        ...
        return [
          pw.Header(
            ...
            child: pw.Row(
              ...
              children: [
                pw.Text('Logo', style: pw.TextStyle(fontSize: 20, color: PdfColors.blue900)),
                pw.Text('تفاصيل المستجدات', style: pw.TextStyle(fontSize: 20, color: PdfColors.blue900)),
              ],
            ),
          ),
          ...
          pw.Directionality(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                pw.Divider(),
                pw.SizedBox(height: 10),
                _buildInfoRow('الفئة:', mostajadat.category),
                _buildInfoRow('تاريخ النشر:', dateFormatter.format(mostajadat.date)),
                if (mostajadat.deadlineDate != null)
                  _buildInfoRow('تاريخ الانتهاء:', dateFormatter.format(mostajadat.deadlineDate!)),
                pw.SizedBox(height: 20),
                pw.Text('المحتوى:', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 10),
                ...mostajadat.contentBlocks.map((block) => _buildContentBlock(block)).toList(),
                pw.SizedBox(height: 20),
                if (mostajadat.buttonLinks.isNotEmpty) ...[
                  pw.Text('روابط إضافية:', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                  pw.SizedBox(height: 10),
                  ...mostajadat.buttonLinks.map((link) => _buildLinkRow(link.text, link.url)),
                ],
                if (mostajadat.iconLinks.isNotEmpty) ...[
                  pw.SizedBox(height: 10),
                  pw.Text('روابط الأيقونات:', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
                  pw.SizedBox(height: 10),
                  ...mostajadat.iconLinks.map((link) => _buildLinkRow(link.iconUrl, link.url)),
                ],
              ],
            ),
          ),
  ...
  try {
    ...
    final file = File('${downloadsDirectory.path}/mostajadat_${mostajadat.id}.pdf');
    await file.writeAsBytes(await pdf.save());

    // Hide the download progress banner
    ScaffoldMessenger.of(context).hideCurrentMaterialBanner();

    // Show a success notification
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('تم تحميل الملف بنجاح في مجلد التنزيلات'),
...
pw.Widget _buildContentBlock(dynamic block) {
  if (block is TextBlock) {
    return pw.Padding(
    ...
    // For simplicity, we're just showing a placeholder for images
    return pw.Container(
    ...
    return pw.Container(
      ...
      child: pw.Text(block.text, style: pw.TextStyle(font: pw.Font.courier())),
    );
  } else if (block is BlockquoteBlock) {
    return pw.Container(
      margin: pw.EdgeInsets.symmetric(vertical: 10),
      padding: pw.EdgeInsets.all(10),
      decoration: pw.BoxDecoration(
        border: pw.Border(right: pw.BorderSide(color: PdfColors.grey400, width: 4)),
      ),
      child: pw.Text(block.text, style: pw.TextStyle(fontStyle: pw.FontStyle.italic)),
    );
  } else {
...
pw.Widget _buildLinkRow(String text, String url) {
  return pw.Padding(
    padding: pw.EdgeInsets.symmetric(vertical: 5),
    child: pw.Row(
      children: [
        pw.Text(text),
        pw.SizedBox(width: 5),
        pw.Text(url, style: pw.TextStyle(color: PdfColors.blue, decoration: pw.TextDecoration.underline)),
      ],
    ),
  );
}
