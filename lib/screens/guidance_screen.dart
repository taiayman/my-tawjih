...
Widget _buildUniversitiesForSpecialization(Specialization specialization) {
  return Container(
    padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: specialization.universities.map((university) => 
        Padding(
          padding: const EdgeInsets.only(bottom: 8.0),
          child: Text(
            university.name,
            style: cairoRegular.copyWith(fontSize: 14, color: Colors.black87),
          ),
        )
      ).toList(),
    ),
...
class _UniversityDetailsScreenState extends State<UniversityDetailsScreen> {
  ...
  ScaffoldMessenger.of(context).showMaterialBanner(
    MaterialBanner(
      content: Text('جاري تحميل معلومات الجامعة...'),
  ...
  // Prepare university image
  pw.ImageProvider? universityImage;
  if (widget.university.imageUrl.isNotEmpty) {
    try {
      final imageBytes = await networkImageToByte(widget.university.imageUrl);
      universityImage = pw.MemoryImage(imageBytes);
    } catch (e) {
      print('Error loading university image: $e');
    }
  }
        ...
        return [
            ...
            child: pw.Row(
              ...
              children: [
                universityImage != null
                    ? pw.Image(universityImage, width: 60, height: 60)
                    : pw.Container(
                        width: 60,
                        height: 60,
                        ...
                        child: pw.Center(
                          child: pw.Text('U', style: pw.TextStyle(fontSize: 30, color: PdfColors.grey600)),
                        ),
                      ),
                pw.Text('معلومات الجامعة', style: pw.TextStyle(fontSize: 20, color: PdfColors.blue900)),
              ],
            ),
          ),
          ...
          pw.Container(
            ...
            child: pw.Column(
              ...
              children: [
                pw.Text(widget.university.name, 
          ...
          pw.Container(
            ...
            child: pw.Text(widget.university.description),
          ),
          pw.SizedBox(height: 20),
          pw.Text('تاريخ إنشاء التقرير: ${DateFormat('dd/MM/yyyy HH:mm').format(DateTime.now())}', 
  ...
  try {
    ...
    final file = File('${downloadsDirectory.path}/university_${widget.university.id}.pdf');
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('تم تحميل معلومات الجامعة بنجاح'),
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('حدث خطأ أثناء تحميل معلومات الجامعة'),
