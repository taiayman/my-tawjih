...
class _MostajadatCustomizationScreenState
  ...
  Widget _buildFormatButton(IconData icon, VoidCallback onPressed, {bool isActive = false}) {
    return Container(
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            child: Icon(icon, size: 20, color: isActive ? Theme.of(context).primaryColor : Colors.grey[800]),
          ),
        ),
      ),
    );
  ...
  void _applyTextStyle({
    ...
    setState(() {
      if (_currentBlockIndex >= 0 && _currentBlockIndex < _contentBlocks.length) {
        ...
        if (block is TextBlock) {
          TextStyle newStyle = block.textStyle;
          
          if (weight != null) {
            newStyle = newStyle.copyWith(
              fontWeight: newStyle.fontWeight == weight ? FontWeight.normal : weight
            );
          }
          
          if (fontStyle != null) {
            newStyle = newStyle.copyWith(
              fontStyle: newStyle.fontStyle == fontStyle ? FontStyle.normal : fontStyle
            );
          }
          
          if (decoration != null) {
            newStyle = newStyle.copyWith(
              decoration: newStyle.decoration == decoration ? TextDecoration.none : decoration
            );
          }
